        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: 'var(--color-bg-subtle)', textAlign: 'left', borderBottom: '1px solid var(--color-border)' }}>
              <th onClick={() => requestSort('first_name')} style={{ padding: '0.5rem 1rem', fontWeight: 500, color: 'var(--color-text-muted)', fontSize: '0.85rem', cursor: 'pointer' }}>
                Name{getSortIndicator('first_name')}
              </th>
              <th onClick={() => requestSort('email')} style={{ padding: '0.5rem 1rem', fontWeight: 500, color: 'var(--color-text-muted)', fontSize: '0.85rem', cursor: 'pointer' }}>
                Email{getSortIndicator('email')}
              </th>
              <th onClick={() => requestSort('intent_status')} style={{ padding: '0.5rem 1rem', fontWeight: 500, color: 'var(--color-text-muted)', fontSize: '0.85rem', cursor: 'pointer' }}>
                Status{getSortIndicator('intent_status')}
              </th>
              <th onClick={() => requestSort('last_contacted_at')} style={{ padding: '0.5rem 1rem', fontWeight: 500, color: 'var(--color-text-muted)', fontSize: '0.85rem', cursor: 'pointer' }}>
            ...
            {sortedSubscribers.length === 0 ? (
              ...
              sortedSubscribers.map(sub => (
                <tr key={sub.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                  ...
                  <td style={{ padding: '0.6rem 1rem', color: 'var(--color-text-muted)', fontSize: '0.9rem' }}>{sub.email}</td>
                  <td style={{ padding: '0.6rem 1rem' }}>
                    <span style={{ 
                      fontSize: '0.7rem', 
